const GRAVITATIONAL_EARTH_PARAMETER = 398600.4418;
...
function getOrbitalPeriod(position, velocity) {
    ...
    return orbitalPeriodSeconds / 60;
}
...
function createSatelliteEntity(viewer, name, tleLine1, tleLine2) {
    ...
    const satelliteEntity = viewer.entities.add({
        ...
        position: new Cesium.CallbackProperty(function(time, result) {
            ...
            if (eci) {
                ...
                // Phải tính gmst thực tế tại thời điểm `time` để chuyển đổi ECI sang ECEF chính xác.
                // Điều này làm cho vệ tinh di chuyển đúng trên bề mặt quả địa cầu đang quay.
    ...
    // Lưu lại các thông tin cần thiết cho việc vẽ quỹ đạo sau này
...
let hasLoggedOrbitPositions = false;

function toggleSatelliteOrbit(satelliteEntity) {
    ...
    try {
        ...
        for (let i = 0; i < lines.length; i += 3) {
            ...
            if (name && tleLine1 && tleLine2) {
                createSatelliteEntity(viewer, name, tleLine1, tleLine2);
            }
        }
        ...
        console.error("Lỗi khi tải hoặc xử lý file TLE:", error);
    }
}
